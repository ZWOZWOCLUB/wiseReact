...
import { callDocumentFileDeleteAPI, callDocumentFileUpdateAPI } from '../../apis/SettingDocumentAPICalls';
...
function SettingDocumentETC() {
    ...
    const onClickDocFileDown = async (index) => {
        try {
            const urlPath =
                'http://localhost:8001' + '/etcDocumentFile/' + prevList.documentFileDTO[index].docAtcConvertName;
            console.log(urlPath);
            ...
            {Array.isArray(prevList.documentFileDTO) && prevList.documentFileDTO.length > 0
                ? prevList.documentFileDTO.map((doc, index) => (
                      <tr style={{ textAlign: 'left' }} className='List' key={index}>
                          ...
                          <td>
                              <label htmlFor='docFile' style={{ paddingRight: 10, cursor: 'pointer' }}>
