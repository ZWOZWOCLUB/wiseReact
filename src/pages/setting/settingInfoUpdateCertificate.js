...
function SettingInfoUpdateCertificate({ onUpdate }) {
  ...
  const handleRemoveRow = (index) => {
    ...
    const findMatchCode = prevList.certificateFileDTO.some(
      (file) => file.cerCode === cerCode
    );

    if (findMatchCode) {
      alert(
        "자격 증명 파일이 등록되어 있어 삭제 불가능합니다. \n 파일 먼저 삭제해 주세요"
      );
    } else {
              ...
              <div className="input-group3" key={index}>
                ...
                <div className="inputWrapper">
                  <input
                    ...
                    name="cerEndDate"
                    value={
                      !updateCerState ? cer.cerEndDate : cerForm.cerEndDate
                    }
                  />
                </div>
                <div className="inputWrapper">
