...
import { callDegreeFileDeleteAPI, callDegreeFileInsertAPI, callDegreeFileUpdateAPI } from "../../apis/SettingDegreeAPICalls";
...
function SettingDocumentDegree() {
  ...
  const DegreeFileUpload = async (degFile, index) => {
    ...
    if (prevList.degreeFileDTO && prevList.degreeFileDTO.length > 0 && prevList.degreeFileDTO[index]) {
            ...
            {Array.isArray(prevList.degreeFileDTO) && prevList.degreeFileDTO.length > 0 && prevList.degreeFileDTO[index] ? (
              <td>
                <button
                  className="bx bx-x"
                  name="btnX"
                  onClick={() => { onClickDegreeFileDelete(index) }}
