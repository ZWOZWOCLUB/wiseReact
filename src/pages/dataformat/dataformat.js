...
function DataFormat() {
  ...
  const handleSearch = (e) => {
    if (e.key === "Enter" || e.type === "click") {
      ...
      let searchResult = [];
      // 제목으로 검색
      searchResult = dataFormatList.filter((dataFormat) =>
        dataFormat.dataName.includes(search)
      );
  ...
  const uploadFile = async (formData) => {
    try {
      const response = await dispatch(
        callDataFormatInsertAPI(formData)
      ).unwrap();
  ...
  const onClickDeleteFile = async (dataCode) => {
    ...
    try {
      ...
      const updatedList = dataFormatList.filter(
        (df) => df.dataCode !== dataCode
      );
            ...
            <div className="content-wrapper">
              <div className="container-xxl flex-grow-1 container-p-y">
                <h4 className="fw-bold py-3 mb-4">
                  <span className="text-muted fw-light">자료실 {">"}</span>{" "}
                  서식자료실
                </h4>
